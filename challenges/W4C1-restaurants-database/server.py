...
def generate_html(restaurants):
    ...
    <html lang="en-GB">
        ...
        <body>
            ...
            <form action="./cgi-bin/restaurants.py">
            </form>
...
def get_restaurants_from_db(neighbourhood):
    ...
    # Get the restaurant name and neighbourhood name out of the database, based on neihgbourhood
    db_cursor.execute(f"""
    SELECT restaurants.NAME,neighborhoods.NAME
    FROM restaurants LEFT JOIN neighborhoods 
    ON restaurants.NEIGHBORHOOD_ID = neighborhoods.ID 
    WHERE restaurants.NEIGHBORHOOD_ID IN 
    (SELECT neighborhoods.ID from neighborhoods 
    WHERE neighborhoods.NAME="{neighbourhood}");
    """)
...
if __name__ == "__main__":
    ...
    restaurants_list = get_restaurants_from_db("Kreuzberg")
